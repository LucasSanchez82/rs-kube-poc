...
pub async fn handle_create_database<'a>(database: DatabaseProvider<'a>) -> Result<String> {
    use k8s_openapi::api::core::v1::Pod;

    println!("{:#?}", database);
    let mut config = Config::infer().await?;
    config.cluster_url = "https://localhost:8443".parse()?;
